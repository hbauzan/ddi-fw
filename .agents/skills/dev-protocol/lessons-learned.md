...
- **Inspección Dual-Engine Extendida (5 Almas × 2 Motores, 2026-09-19)**: En mazos crudos extendidos de 110 cláusulas completas sin podar (`ddi_fw/data/extended/`), tanto BGE-M3 (1024-D) como Qwen2-1.5B (1536-D) registran `disjoint_count = 0` en los 10 pares canónicos (`ok_unpublished`), respetando estrictamente la invariante de fail-closed y absteniéndose de forzar $\epsilon$-gaps artificiales. Aislamiento por subprocesos garantizó que la huella pico de memoria (RSS 907 MB en BGE-M3, 4206 MB en Qwen2) se purgara totalmente entre corridas. Artefactos exportados con precisión IEEE 754 float32 nativa (`f'{val:.17g}'`) y cálculos con `Decimal` en `ddi_fw/out/extended_bge/` y `ddi_fw/out/extended_qwen2/`. El ledger comparativo reside en `current-research/engines/dual-engine-extended-inspection.md`.
