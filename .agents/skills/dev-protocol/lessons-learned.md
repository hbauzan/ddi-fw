...
- **Validación Adversarial y Fuzzing Espectral con Rompepepe (2026-09-26)**:
  - *Integración con el Proxy v0.4.0*: `FirewallClient` interactúa con el endpoint OpenAI-compatible estándar `POST /v1/chat/completions` y sondea `GET /healthz`. Las brechas en ingress responden HTTP 403 `ddi_ingress_breach` con telemetría espectral en `error.audit` (`trigo_votes`, `ruido_votes`, `quorum_reached`, `quorum_min`). Respuestas HTTP 200 o HTTP 502 (`ddi_upstream_error`) señalan que la inspección de ingress fue superada y el payload se intentó remitir al backend destino.
  - *Resiliencia de Quórum bajo Ataques Adversariales*: Campaña de 250 evaluaciones grid-search sobre 125 vectores en 5 categorías (piggybacking semántico, mutaciones de frontera, inyección trilingüe cross-domain, quórum stress y Python legítimo) más 15 iteraciones de mutación heurística adaptativa confirmaron **0 bypasses empíricos** (tasa de contención del 100.0%), ratificando la cota teórica $P_{\text{bypass}} \le (0.80)^{100} \approx 2.037 \times 10^{-10} < 10^{-9}$.
