...
- [ ] `ddi_fw/out/qwen2/rows.npz` + `press.json` + `measure_audit.json` exist locally (gitignored).
- [ ] Engine dump has the three pairs filled with one of `ok_published` / `ok_unpublished` / `blocker_*`.
- [ ] `dropped` is [].
