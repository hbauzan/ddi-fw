...
## 2. Criterio A — Paja por Saturación / Magnitud Universal
...
### Regla de Detección
Una dimensión $d$ se clasifica como **Paja por Saturación ($P_A$)** si:
...
## 3. Criterio B — Paja por Indiferenciación Temática ($\Delta \approx 0$)
...
### Regla de Detección
Una dimensión $d$ se clasifica como **Paja por Indiferenciación ($P_B$)** si:
...
## 4. Clasificación y Catálogo de la Paja

Toda dimensión analizada se etiquetará en una de las siguientes categorías:

| Clasificación | Definición | Acción |
| :--- | :--- | :--- |
| **`PAJA_AMBOS`** | Cumple Criterio A (Saturación) y Criterio B (Indiferenciación) | Poda prioritaria |
| **`PAJA_SATURADA`** | Cumple solo Criterio A (Alta energía universal) | Poda por ruido dominante |
| **`PAJA_PLANA`** | Cumple solo Criterio B ($\Delta \approx 0$ entre temas) | Poda por falta de información |
| **`TRIGO_CANDIDATO`** | No cae en ningún criterio de paja | Pasa a formar el extracto puro |
...
1. **Catálogo de Paja:**
   * `catalogo_paja_estructural.csv`: Listado de las 1024 dimensiones con sus valores en los 5 temas, distancia máxima entre centros y etiqueta final de paja/trigo.
   * `catalogo_paja_estructural.json`: Metadata con conteos absolutos, umbrales utilizados y ficha de hardware.
